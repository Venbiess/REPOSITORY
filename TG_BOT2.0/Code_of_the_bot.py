...
'''
ПЕРЕД ЗАПУСКОМ: 
1. Установить следующий файл на свой компьютер: vectors.nlpl.eu/repository/20/182.zip
2. Полодить рядом с кодом в оду папку.
...
#=======================================
#подготовка модели
#Послденяя, работающая версия: /content/drive/My Drive/БД/180/model.txt
f = open('nltk_data/Проект/model.txt', encoding='utf-8')
for line in f:
    values = line.split()
    word = values[0]
...
def text_process(message):
    global text
    text  = message.text; 
    #Создадим список, в котором будет хранится значение вектора каждого предложения
    sentence_vectors = []
    #Создадим список, в котором будуд хранится предложения, которые прошли препроцессинг
    clean_sentences = []
    #Токенизируем текст
    sentences = sent_tokenize(text, language = 'russian')
    a = sentences
    #Препроцессинг текста: лемитизация.
    for k in range(0, len(sentences)):
      new_sent = process(process_pipeline, sentences[k], keep_pos=True, keep_punct=False)
      if (len(new_sent) != 0):
    ...
    #Расчет вектора каждого из предложений.
    count = 0;
    for i in clean_sentences:
      ...
      count = count + 1;
    #print(len(sentenses))
    #Матрица подобия предложений. Вычисление косинусного расстояния у каждой пары предложений. 
    ...
    #Составление рейтинга предложений по их значимости в тексте. 
    ranked_sentences = sorted(((scores[i],s) for i,s in enumerate(sentences)), reverse=True)

    import math
    if (len(sentences) > 2):
      sn = math.ceil(0.2*len(sentences))
    else:
      sn = 1
    ranked_output = []
    # Создание пересказа.
    for i in range(sn):
      ...
      ranked_output.append(str(ranked_sentences[i][1]))
    #Вывод итогового пересказа в соответсвии с исходным текстом. 
    output_summary = []
    for k in range(len(sentences)):
      for l in range(len(ranked_output)):
        if (str(sentences[k]) == str(ranked_output[l])):
          output_summary.append(str(sentences[k]))
    string = ' '.join(output_summary)
    #return ((string))
...
def start(message):
    if message.text == '/text':
        ...
        bot.register_next_step_handler(message, text_process); #следующий шаг – функция get_text
    else:
